...
class vsqx_option():
    ...
    def start_doing(self):
        ...
        if self.path is None:
            ...
            tree = self.tree
        self.singers = tree.find('./%svVoiceTable' % namespace).findall(
            '%svVoice' % namespace)
        s_bs = []
        s_pc = []
        s_name = []
        for singer in self.singers:
            ...
            s_name.append(
                singer.find('%sname' % namespace).text.replace('@12@',
                                                               '').replace(
                                                                   '@13@', ''))
#
#  Find Tracks
#
        ...
        for track in self.tracks:
            print()
            print("[+] " + str(trackN))
            print("[-] Name    =" + track.find('%sname' % namespace)
                  .text.replace('@12@', '').replace('@13@', ''))
            print("[-] Comment =" + track.find('%scomment' % namespace)
                  .text.replace('@12@', '').replace('@13@', ''))
            trackN += 1
        print()
        try:
...
#
        ...
        for part in self.parts:
            print()
            print("[+] " + str(partN))
            print("[-] Name    =" + part.find('%sname' % namespace)
                  .text.replace('@12@', '').replace('@13@', ''))
            print("[-] Comment =" + part.find('%scomment' % namespace)
                  .text.replace('@12@', '').replace('@13@', ''))
            for i in range(len(s_pc)):
                if part.find('%ssinger' % namespace).find(
                        '%spc' % namespace).text == s_pc[i]:
...
#
        ...
        for note in self.notes:
            ...
            printstr = "[" + str(noteN) + ']' + 'LRC=' + note.find(
                '%sy' % namespace).text.replace('@12@', '').replace(
                    '@13@', '')
            printstr += "  PHONETIC=" + note.find(
                '%sp' % namespace).text.replace('@12@', '').replace(
                    '@13@', '')
            printstr += "  LOCATION=" + note.find(
                '%st' % namespace).text.replace('@12@', '').replace(
                    '@13@', '')
            printstr += "  TONE=" + note.find('%sn' % namespace).text
            print(printstr)
            noteN += 1
        return chose_part

# Function: APPEND NOTEs:
# Required: Element_path, location, lrc, phonetic, vel, length, tone, accent
    ...
    def append_note(
            ...
            vibLen=66,
            vibType=1,
            path="./%svsTrack/%svsPart" % (namespace, namespace),
    ):
...
#Advanced Options
    ...
    def inspire_phonetic(self, selected_part, phonetic, begin=0, end=None):
        part = selected_part
...
def interface_5(vocaloid, part):
    ...
    print("[-] Lyric    =" + note.find('%sy' % namespace).text.replace(
        '@12@', '').replace('@13@', ''))
    print("[-] Phonetic =" + note.find('%sp' % namespace).text.replace(
        '@12@', '').replace('@13@', ''))
    print("[-] Location =" + note.find('%st' % namespace).text)
    print("[-] Length   =" + note.find('%sdur' % namespace).text)
    print("[-] Tone     =" + note.find('%sn' % namespace).text)
    print("[-] VEL      =" + note.find('%sv' % namespace).text)
    print("[-] OPE      =" + note.find('%snStyle' % namespace).findall(
        "./%sv" % namespace)[5].text)
    ...
    vocaloid.change_note(
        note=note, y=y, p=p, t=t, dur=dur, n=n, v=v, nStyles=nStyles)
...
def interface_2(v, selected_part):
    ...
    while choice != '0':
        ...
        if choice is '0':
            return
        v.inspire_phonetic(selected_part, p, begin)
...
def interface_9(v, original_path):
    ...
    if choice is '1':
        ...
        path = input(
            "File name(you needn't add '.vsqx' at the end of filename):")
        path += '.vsqx'
    else:
